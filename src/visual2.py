...
def nmf(path, dest):
    ...
    WF0 = np.load('WF.npy')
    WPHI_voc =np.load('WPHI_sax.npy')
    WPHI_bg = np.load('WPHI_nonsax.npy')
    SX = stuff['arr_0']
...
def nmf_multi(path,dest = './outs_violin/'):
...
if __name__ == '__main__':
    np.random.seed(0)
    path = './stuffs_voc/'
    dest = './out_06-DieSonne_split_sax.npz'

    #nmf_multi(path)
    nmf('./06-DieSonne.npz', dest)
